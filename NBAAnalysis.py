...
# Filter data
filtered_df = nba_logs_df[(nba_logs_df['Date'] >= start_date) & (nba_logs_df['Date'] <= end_date)]
...
threshold = st.sidebar.number_input("Set Threshold", min_value=0.0, max_value=float(max_val if max_val else 100.0),
                                    value=float(default_thresh), step=0.5)
